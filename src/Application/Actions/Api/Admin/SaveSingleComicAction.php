...
final readonly class SaveSingleComicAction implements ActionInterface
{
    ...
    public function execute(ServerRequest $request): mixed
    {
        ...
        try {
            ...
            $urls = $this->resolveUrls($dto->originalUrl, $dto->sketchUrl);
            $originalUrl = $urls['originalUrl'];
            $sketchUrl = $urls['sketchUrl'];

            $charIds = $this->extractCharacterIds($dto->characterIds);

            $oldIdStrRaw = $request->post['old_comic_id'] ?? '';
            $oldIdStr = \is_scalar($oldIdStrRaw) ? \trim((string) $oldIdStrRaw) : '';
            $this->handleRenaming($oldIdStr, $dto->id);

            $hasNewImage = $this->handleMediaUpload($request->files, $dto->id);
    ...
    /**
     * @return array{originalUrl: string, sketchUrl: string}
     */
    private function resolveUrls(string $originalUrl, string $sketchUrl): array
    {
     ...
     */
    private function extractCharacterIds(array $rawIds): array
    {
        $charIds = [];
        foreach ($rawIds as $cId) {
            ...
            try {
                ...
                // Ignorieren, falls das DTO kaputte Strings durchlässt
            }
        }
    ...
    private function handleRenaming(string $oldIdStr, string $newIdStr): void
    {
        // --- DEEP RENAMING LOGIK ---
     ...
     */
    private function handleMediaUpload(array $files, string $comicIdStr): bool
    {
        $hasNewImage = false;
