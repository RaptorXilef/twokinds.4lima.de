(function (global, factory) {
    ...
    class Diff {
        diff(
            ...
            const oldTokens = this.removeEmpty(
                this.tokenize(oldString, options)
            );
            const newTokens = this.removeEmpty(
                this.tokenize(newString, options)
            );
                ...
                for (
                    ...
                    if (addPath) {
                        ...
                        canAdd =
                            addPath &&
                            0 <= addPathNewPos &&
                            addPathNewPos < newLen;
                    }
                    const canRemove =
                        removePath && removePath.oldPos + 1 < oldLen;
                    if (!canAdd && !canRemove) {
        ...
        if (string.slice(0, oldPrefix.length) != oldPrefix) {
            throw Error(
                `string ${JSON.stringify(
                    string
                )} doesn't start with prefix ${JSON.stringify(
                    oldPrefix
                )}; this is a bug`
            );
        }
    ...
    function replaceSuffix(string, oldSuffix, newSuffix) {
        ...
        if (string.slice(-oldSuffix.length) != oldSuffix) {
            throw Error(
                `string ${JSON.stringify(
                    string
                )} doesn't end with suffix ${JSON.stringify(oldSuffix)}; this is a bug`
            );
        }
    ...
    function dedupeWhitespaceInChangeObjects(
        ...
        if (deletion && insertion) {
            ...
            if (startKeep) {
                const commonWsPrefix = longestCommonPrefix(
                    oldWsPrefix,
                    newWsPrefix
                );
            ...
            if (endKeep) {
                const commonWsSuffix = longestCommonSuffix(
                    oldWsSuffix,
                    newWsSuffix
                );
                endKeep.value = replacePrefix(
                    endKeep.value,
                    newWsSuffix,
                    commonWsSuffix
                );
                ...
                Object.assign(Object.assign({}, hunk), {
                    lines: hunk.lines.map((line, i) => {
                        ...
                        return line.startsWith('\\') ||
                            line.endsWith('\r') ||
                            ((_a = hunk.lines[i + 1]) === null || _a === void 0
                                ? void 0
                                : _a.startsWith('\\'))
                            ? line
                            : line + '\r';
    ...
    function isWin(patch) {
        ...
        return (
            patch.some((index) =>
                index.hunks.some((hunk) =>
                    hunk.lines.some((line) => line.endsWith('\r'))
                )
            ) &&
            patch.every((index) =>
                index.hunks.every((hunk) =>
                    hunk.lines.every((line, i) => {
                        var _a;
                        return (
                            line.startsWith('\\') ||
                            line.endsWith('\r') ||
    ...
    function parsePatch(uniDiff) {
        ...
        function parseIndex() {
            ...
            while (i < diffstr.length) {
                ...
                // Diff index
                const header = /^(?:Index:|diff(?: -r \w+)+)\s+(.+?)\s*$/.exec(
                    line
                );
                if (header) {
            ...
            while (i < diffstr.length) {
                ...
                if (
                    ...
                    throw new Error(
                        'Unknown line ' + (i + 1) + ' ' + JSON.stringify(line)
                    );
                } else {
                    i++;
                }
        ...
        function parseHunk() {
            ...
            for (
                ...
                if (
                    ...
                    throw new Error(
                        `Hunk at line ${chunkHeaderIndex + 1} contained invalid line ${
                            diffstr[i]
                        }`
                    );
                }
    ...
    function applyStructuredPatch(source, patch, options = {}) {
        if (
            options.autoConvertLineEndings ||
            options.autoConvertLineEndings == null
        ) {
        ...
        function applyHunk(
            ...
            for (; hunkLinesI < hunkLines.length; hunkLinesI++) {
                const hunkLine = hunkLines[hunkLinesI],
                    ...
                    content =
                        hunkLine.length > 0 ? hunkLine.substr(1) : hunkLine;
                if (operation === '-') {
                    if (
    ...
    function applyPatches(uniDiff, options) {
        const spDiff =
            typeof uniDiff === 'string' ? parsePatch(uniDiff) : uniDiff;
        let currentIndex = 0;
                ...
                if (current.added || current.removed) {
                    ...
                    if (!oldRangeStart) {
                        ...
                        if (prev) {
                            curRange =
                                context > 0
                                    ? contextLines(prev.lines.slice(-context))
                                    : [];
    ...
    function formatPatch(patch) {
        ...
        ret.push(
            '--- ' +
                patch.oldFileName +
                (typeof patch.oldHeader === 'undefined'
                    ? ''
                    : '\t' + patch.oldHeader)
        );
        ret.push(
            '+++ ' +
                patch.newFileName +
                (typeof patch.newHeader === 'undefined'
                    ? ''
                    : '\t' + patch.newHeader)
        );
