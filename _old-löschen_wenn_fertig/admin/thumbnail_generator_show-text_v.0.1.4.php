 ...
 * Es durchsucht das Verzeichnis nach PNG-, GIF- und JPG-Dateien, generiert für jedes Bild ein Thumbnail
 ...
 * Am Ende werden die erstellten Thumbnails angezeigt.
...
// Eingabebilder im PNG-Format, GIF-Format und JPG-Format abrufen
...
$files = array_merge($pngFiles, $gifFiles, $jpgFiles);
...
/*if (!empty($createdThumbnails)) {
    ...
    foreach ($createdThumbnails as $thumbnail) {
        // Ausgabe des Dateinamens des Thumbnails
        $thumbnailFilename = basename($thumbnail);
        echo "<p>{$thumbnailFilename}</p>" . PHP_EOL;
    }
}*/
