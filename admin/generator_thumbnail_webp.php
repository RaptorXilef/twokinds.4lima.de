...
{
    $thumbnailIds = [];
    $imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'webp']; // Prüfe auf gängige Bild-Erweiterungen, inkl. WebP
...
function generateThumbnail(string $comicId, string $lowresDir, string $hiresDir, string $thumbnailDir, bool $debugMode): array
{
    ...
    try {
        ...
        // Speichere den Alphakanal (Transparenz) für das Bild.
        ...
        $thumbnailPath = $thumbnailDir . $comicId . '.webp';
        // Speichere das Bild als WebP mit verlustfreier Kompression (Qualität 101).
        if (imagewebp($tempImage, $thumbnailPath, 101)) {
            $createdPath = $thumbnailPath;
            if ($debugMode)
                error_log("DEBUG: Thumbnail erfolgreich als WebP gespeichert: " . $thumbnailPath);
        } else {
            $errors[] = "Fehler beim Speichern des WebP-Thumbnails für Comic-ID '$comicId' nach '$thumbnailPath'.";
            error_log("Fehler beim Speichern des WebP-Thumbnails für Comic-ID '$comicId' nach '$thumbnailPath'.");
            if ($debugMode)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_single_thumbnail') {
    ...
    if (empty($result['errors'])) {
        ...
        $response['imageUrl'] = '../assets/comic_thumbnails/' . $comicId . '.webp?' . time(); // Cache-Buster
