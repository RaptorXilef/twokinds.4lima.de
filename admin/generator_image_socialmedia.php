...
if ($debugMode)
    error_log("DEBUG: generator_image_socialmedia.php wird geladen.");

// Starte den Output Buffer als ALLERERSTE Zeile, um wirklich jede Ausgabe abzufangen.
ob_start();
if ($debugMode)
    error_log("DEBUG: Output Buffer in generator_image_socialmedia.php gestartet.");
...
session_start();
if ($debugMode)
    error_log("DEBUG: Session gestartet in generator_image_socialmedia.php.");
...
if (!isset($_SESSION['admin_logged_in']) || $_SESSION['admin_logged_in'] !== true) {
    if ($debugMode)
        error_log("DEBUG: Nicht angemeldet, Weiterleitung zur Login-Seite von generator_image_socialmedia.php.");
...
if ($debugMode)
    error_log("DEBUG: Admin in generator_image_socialmedia.php angemeldet.");
...
if ($debugMode) {
    error_log("DEBUG: Pfade definiert: lowresDir=" . $lowresDir . ", hiresDir=" . $hiresDir . ", socialMediaImageDir=" . $socialMediaImageDir);
}
...
if (!extension_loaded('gd')) {
    ...
    error_log("FEHLER: GD-Bibliothek nicht geladen in socialmedia_image_generator.php");
    if ($debugMode)
...
{
    if ($debugMode)
        error_log("DEBUG: getExistingComicIds() aufgerufen für lowres: " . $lowresDir . " und hires: " . $hiresDir);
    $comicIds = [];
        ...
        if ($files === false) {
            ...
            foreach ($files as $file) {
                ...
                if (isset($fileInfo['extension']) && in_array(strtolower($fileInfo['extension']), $imageExtensions)) {
                    ...
                    if ($debugMode)
                        error_log("DEBUG: Comic-ID (lowres) gefunden: " . $fileInfo['filename']);
                }
            }
        }
    } else {
        if ($debugMode)
            error_log("DEBUG: lowresDir nicht gefunden oder ist kein Verzeichnis: " . $lowresDir);
    }
        ...
        if ($files === false) {
            ...
            foreach ($files as $file) {
                ...
                if (isset($fileInfo['extension']) && in_array(strtolower($fileInfo['extension']), $imageExtensions)) {
                    ...
                    if ($debugMode)
                        error_log("DEBUG: Comic-ID (hires) gefunden: " . $fileInfo['filename']);
                }
            }
        }
    } else {
    ...
    if ($debugMode)
        error_log("DEBUG: " . count($comicIds) . " vorhandene Comic-IDs insgesamt gefunden.");
 ...
 */
function generateSocialMediaImage(string $comicId, string $lowresDir, string $hiresDir, string $socialMediaImageDir, bool $debugMode): array
{
    if ($debugMode)
    ...
    foreach ($possibleExtensions as $ext) {
        ...
        if (file_exists($hiresPath)) {
            ...
            if ($debugMode)
                error_log("DEBUG: Quellbild (hires) gefunden: " . $sourceImagePath);
            break;
        } elseif (file_exists($lowresPath)) {
            $sourceImagePath = $lowresPath;
            $sourceImageExtension = $ext;
            if ($debugMode)
                error_log("DEBUG: Quellbild (lowres) gefunden: " . $sourceImagePath);
            break;
        }
    }
    ...
    try {
        ...
        if ($debugMode)
            error_log("DEBUG: Skalierungsfaktor: " . $scale . ", Neue Dimensionen: " . $newWidth . "x" . $newHeight . "px.");
        ...
        if ($debugMode)
            error_log("DEBUG: Offsets für Zentrierung: X=" . (int) $offsetX . ", Y=" . (int) $offsetY);
        ...
        if (
            ...
            error_log("FEHLER: Fehler beim Resampling des Bildes für Comic-ID '$comicId'.");
            imagedestroy($sourceImage);
            imagedestroy($tempImage);
            return ['created' => $createdPath, 'errors' => $errors];
        }
        if ($debugMode)
            error_log("DEBUG: Bild resampled und kopiert.");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_single_social_media_image') {
    ...
    $result = generateSocialMediaImage($comicId, $lowresDir, $hiresDir, $socialMediaImageDir, $debugMode);
...
if (file_exists($headerPath)) {
    include $headerPath;
    if ($debugMode)
        error_log("DEBUG: Header in generator_image_socialmedia.php eingebunden.");
} else {
...
<article>
    ...
    <div class="content-section">
        ...
        <?php if (empty($allComicIds)): ?>
            ...
            <?php if ($debugMode)
                error_log("DEBUG: Keine Comic-Bilder in lowres/hires gefunden, Statusmeldung angezeigt."); ?>
...
if (file_exists($footerPath)) {
    include $footerPath;
    if ($debugMode)
        error_log("DEBUG: Footer in generator_image_socialmedia.php eingebunden.");
} else {
