...
document.addEventListener('DOMContentLoaded', () => {
        ...
        if (reportType === 'transcript') {
            ...
            if (suggestion) {
                if (suggestionHtmlDisplay)
                    suggestionHtmlDisplay.innerHTML = suggestion;
                if (suggestionCodeTextarea)
                    suggestionCodeTextarea.value = suggestion;
            } else {
                if (suggestionHtmlDisplay)
                    suggestionHtmlDisplay.innerHTML =
                        '<em>Kein Vorschlag (HTML).</em>';
                if (suggestionCodeTextarea)
    ...
    const generateTextDiff = (originalHtml, suggestionHtml) => {
        ...
        if (
            ...
            typeof Diff.diffLines === 'undefined'
        ) {
            diffViewer.innerHTML =
        ...
        diff.forEach((part) => {
            ...
            if (part.value.match(/^\s+$/)) {
                ...
                node.appendChild(
                    document.createTextNode('[Whitespace-Änderung]')
                );
            } else {
