...
final readonly class GoogleAnalyticsClient implements AnalyticsClientInterface
{
    ...
    public function trackPageView(string $clientId, string $sessionId, string $pageLocation, string $pageTitle): void
    {
        ...
        $apiUrl = 'https://www.google-analytics.com/mp/collect?measurement_id=' . \urlencode($gaId) . '&api_secret=' . \urlencode($apiSecret); // phpcs:ignore Generic.Files.LineLength.TooLong
        $ch = \curl_init($apiUrl);
        if ($ch === false) {
            return;
        }
